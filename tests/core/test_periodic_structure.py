...
def test_simple_structure_has_correct_sites(
...
def test_simple_structure_doesnt_have_incorrect_sites(
    square_2x2_2D_grid_in_test: PeriodicStructure,
):
    assert square_2x2_2D_grid_in_test.site_at((0.5, 0.501)) is None


def test_simple_structure_has_periodic_sites(
    square_2x2_2D_grid_in_test: PeriodicStructure,
):
    assert square_2x2_2D_grid_in_test.site_at((-1.5, -1.5)) is not None
    assert square_2x2_2D_grid_in_test.site_at((-1.5, 1.5)) is not None
...
def test_build_structure_from_list_motif(square_2D_lattice):
    ...
    struct = PeriodicStructure.build_from(
        square_2D_lattice, num_cells=(3, 3), site_motif=motif
    )
