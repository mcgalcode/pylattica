...
def test_analyze_count_equal(grid_setup: DiscreteGridSetup):
    ...
    assert (
        analyzer.get_site_count_where_equal(
            periodic_state.state, {DISCRETE_OCCUPANCY: "A"}
        )
        == 8
    )
    ...
    assert (
        analyzer.get_site_count_where_equal(
            periodic_state.state, {DISCRETE_OCCUPANCY: "B"}
        )
        == 8
    )
    ...
    assert (
        analyzer.get_site_count_where_equal(
            periodic_state.state, {DISCRETE_OCCUPANCY: "C"}
        )
        == 0
    )
...
def test_analyze_get_sites_where_equal(grid_setup: DiscreteGridSetup):
    ...
    a_site_ids = analyzer.get_sites_where_equal(
        periodic_state.state, {DISCRETE_OCCUPANCY: "A"}
    )
    ...
    b_site_ids = analyzer.get_sites_where_equal(
        periodic_state.state, {DISCRETE_OCCUPANCY: "B"}
    )
