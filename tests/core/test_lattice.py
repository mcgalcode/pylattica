...
def test_rectangular_lattice_point_conversions():
    ...
    assert (
        np.array([1 / 2, 1 / 4]) == lattice.get_periodized_cartesian_coords(pt1)
    ).all()
    assert (
        np.array([1 / 2, 1 / 4]) == lattice.get_periodized_cartesian_coords(pt2)
    ).all()
...
def test_scaled_rectangular_lattice_point_conversions():
    ...
    # periodizing cartesian points
    assert (
        np.array([3 / 2, 1 / 4]) == lattice.get_periodized_cartesian_coords(pt1)
    ).all()
    assert (
        np.array([1 / 2, 1 / 4]) == lattice.get_periodized_cartesian_coords(pt2)
    ).all()
...
def test_square_lattice_non_pbc_distance():
    lattice = Lattice([[1, 0], [0, 1]], periodic=False)
...
def test_canted_rectangular_lattice_point_conversions():
    ...
    assert (
        np.array([3 / 2, 3 / 4]) == lattice.get_periodized_cartesian_coords(pt2)
    ).all()
...
def test_nonperiodic_lattice(square_2D_basis_vecs):
    ...
    assert_points_equal(
        lat_slab.get_periodized_cartesian_coords([1.5, 1.5]), [0.5, 1.5]
    )
