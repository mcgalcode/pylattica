...
def test_can_run_growth_sim_parallel():
    ...
    periodic_initial_state = setup.setup_coords(20, "A", {"B": [(10, 10)]})
    ...
    assert (
        analyzer.get_site_count_where_equal(res.get_step(1), {DISCRETE_OCCUPANCY: "B"})
        == 9
    )

    assert (
        analyzer.get_site_count_where_equal(res.get_step(2), {DISCRETE_OCCUPANCY: "B"})
        == 25
    )
