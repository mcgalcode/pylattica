...
def test_setup_random_particles(grid_setup: DiscreteGridSetup):
    state = grid_setup.setup_random_particles(
        4, radius=2, num_particles=3, bulk_phase="A", particle_phases=["B"]
    )
...
def test_setup_random_sites(grid_setup: DiscreteGridSetup):
    ...
    simulation = grid_setup.setup_random_sites(
        4, num_sites_desired=num_sites, background_spec="A", nuc_amts=nuc_amts, buffer=1
    )
    ...
    assert (
        analyzer.get_site_count(simulation.state, state_criteria=[count_criteria])
        == num_sites
    )
...
def test_setup_specific_coords(grid_setup: DiscreteGridSetup):
    simulation = grid_setup.setup_coords(
        4, background_state="A", coordinates={"B": [[0, 0]], "C": [[1, 1]]}
    )
