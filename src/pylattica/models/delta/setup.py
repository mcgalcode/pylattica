...
class DeltaSetup:
    def setup(self, struct, ratios):
        ...
        spec_assignments = [
            spec
            for count, spec in zip(site_counts, species)
            for _ in range(ceil(count))
        ]
        ...
        for site in struct.sites(OCT_SITE):
            ...
            state.set_site_state(
                site["id"], {Occupancy: spec_assignments_queue.popleft()}
            )

        for site in struct.sites(TET_SITE):
            state.set_site_state(site["id"], {Occupancy: Vacant})

        state.set_general_state(
            {
                Mn2Count: 0,
                InitialLiCount: analyzer.get_site_count(
                    state, state_true_criteria={Occupancy: Li}
                ),
                InitialAtomCount: analyzer.get_site_count(
                    state, state_false_criteria={Occupancy: Vacant}
                ),
                RemovedLiCount: 0,
            }
        )
